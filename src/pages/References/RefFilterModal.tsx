...
const RefFilterModal = ({
  ...
  const dateField = (key: string, caption: string) => (
    <div className="relative">
      <span className="pointer-events-none absolute left-3.5 top-1.5 z-10 text-[11px] text-[#a3a3a3] dark:text-[#525252]">
        {caption}
      </span>
      <DatePicker
        value={values[key]}
        onChange={(d: Date | null) => setVal(key, d)}
        showTime
        widthClass="w-full"
        inputClassName="h-14 items-stretch [&>input]:h-auto [&>input]:pt-5 [&>input]:pb-1"
  ...
  const dateRangeBlock = (afterKey: string, beforeKey: string, label: string) => (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 mt-2">
        {dateField(afterKey, "dan")}
        {dateField(beforeKey, "gacha")}
      </div>
    </div>
  );
